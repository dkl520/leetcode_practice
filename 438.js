...
//         let sliceStr = s.slice(i, i + p.length).split("");
//         sliceStr = sliceStr.sort().join("");
//         let validBol = (sliceStr == p);
//         if (validBol) {
//             resultList.push(i)
//         }
//     }
...
//         if (validBol) {
//             resultList.push(i);
//             for (let j = p.length + i; j < s.length; j++) {
//                 const element = s[j];
//                 const prev = i;
//                 if (s[prev] == element) {
//                     resultList.push(i + 1)
...
// };
// 滑动窗口
// 定义一个名为findAnagrams的函数，接受两个参数，一个源字符串s和一个目标字符串p  
var findAnagrams = function (s, p) {
    // 获取源字符串和目标字符串的长度  
    const sLen = s.length, pLen = p.length;
    // 如果源字符串长度小于目标字符串长度，则无法找到anagram，直接返回空数组  
    if (sLen < pLen) {
        return [];
    }
    
    // 初始化一个名为ans的数组，用于存储找到的anagram的开始索引  
    const ans = [];
    // 初始化两个计数数组，sCount用于记录源字符串中每个字符出现的次数，pCount用于记录目标字符串中每个字符出现的次数  
    const sCount = new Array(26).fill(0); // 这里假设只处理小写英文字母，所以数组长度是26  
    const pCount = new Array(26).fill(0);
    // 遍历目标字符串，对每个字符进行计数  
    for (let i = 0; i < pLen; ++i) {
        ++sCount[s[i].charCodeAt() - 'a'.charCodeAt()]; // 对源字符串中的字符进行计数  
        ++pCount[p[i].charCodeAt() - 'a'.charCodeAt()]; // 对目标字符串中的字符进行计数  
    }
    // 如果源字符串和目标字符串的字符出现次数完全相同，那么第一个anagram的开始索引就是0  
    if (sCount.toString() === pCount.toString()) {
        ans.push(0);
    }
    // 从源字符串的第二个字符开始遍历，每次移动pLen个位置  
    
    for (let i = 0; i < sLen - pLen; ++i) {

        --sCount[s[i].charCodeAt() - 'a'.charCodeAt()]; // 对源字符串中的字符计数减一  
        ++sCount[s[i + pLen].charCodeAt() - 'a'.charCodeAt()]; // 对源字符串中的字符计数加一  

        // 如果源字符串和目标字符串的字符出现次数完全相同，那么这个位置就是一个anagram的开始索引  
        if (sCount.toString() === pCount.toString()) {
            ans.push(i + 1);
        }
    }
    // 返回所有找到的anagram的开始索引数组  
    return ans;
};
...
console.time("38. 找到字符串中所有字母异位词")
console.log(findAnagrams(s, p));
console.timeEnd("38. 找到字符串中所有字母异位词")
